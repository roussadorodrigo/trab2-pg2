...
int main(int argc, char * argv[]){
	...
	char* isbn;	
	char* author_name;													// String que tem o isbn apenas proveniente do "Array de input"
	...
	while(1){
		...
		switch(input[0]){	
			...
			case 'i':													/// Comando i
				...
				case 'a':												/// Comando a
				author_name = strtok(input, " ");
				author_name = strtok(NULL, " ");
				separatorUnify(author_name);
				
				
				for(int i = 0; author_name[i] != '\0'; i++){			//Ciclo para manter apenas caracteres do alfa <--> função?
					if(!(isalpha(author_name[i]))) author_name[i] = author_name[i+1];
				}
				
				
				int num = 0;
				for(int i = 0; i <= col.count-1; i++){
					if(bookContainsAuthor(&(col.books[i]), author_name)){
						printf("\n TITLE: %s | ", col.books[i].title);
						printf("ISBN: %s | ", col.books[i].isbn);
						printf("AUTHORS: %s | ", col.books[i].authors);
						printf("PUBLISHER: %s\n\n ", col.books[i].publisher);
				 ...
				 printf("\n %d books found with \"%s\" in its authors data\n\n ", num, author_name);
